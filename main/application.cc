...
#error "CONFIG_USE_DEVICE_AEC and CONFIG_USE_SERVER_AEC cannot be enabled at the same time"
    ...
    esp_timer_create_args_t alarm_timer_args = {
        .callback = [](void* arg) {
            Application* app = (Application*)arg;
            if (app->GetDeviceState() == kDeviceStateAlarm) {
                app->audio_service_.PlaySound(Lang::Sounds::OGG_RINGTONE);
            }
        },
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
...
Application::~Application() {
    ...
    if (alarm_timer_handle_ != nullptr) {
        esp_timer_stop(alarm_timer_handle_);
        esp_timer_delete(alarm_timer_handle_);
    }
    if (alarm_stop_timer_handle_ != nullptr) {
        esp_timer_stop(alarm_stop_timer_handle_);
        esp_timer_delete(alarm_stop_timer_handle_);
    }
...
void Application::Start() {
    ...
    // Start fetch audio loop task for podcast mode
    xTaskCreate([](void* arg) {
        Application* app = (Application*)arg;
        app->FetchAudioLoop();
        vTaskDelete(NULL);
            ...
            if (cJSON_IsString(state)) {
                if (strcmp(state->valuestring, "start") == 0 || strcmp(state->valuestring, "resume") == 0) {
                    ...
                    if (cJSON_IsString(name)) {
                        ESP_LOGI(TAG, "Podcast started: %s", name->valuestring);
                    ...
                    ESP_LOGE(TAG, "Podcast playback error");
                    Schedule([this, display]() {
                    ...
                    ESP_LOGI(TAG, "Podcast prompt received, waiting for audio to finish...");
                    Schedule([this, display]() {
                        // Wait a bit for audio service to finish current playback
                        vTaskDelay(pdMS_TO_TICKS(3000));
...
void Application::ScheduleEvent(uint32_t event_bits) {
...
void Application::FetchAudioLoop() {
    Ota ota;
    while (true) {
        if (podcast_mode_ && device_state_ != kDeviceStateFatalError) {
            vTaskDelay(pdMS_TO_TICKS(1000 * 60));
            continue;
        }
        bool has_new_audio = ota.CheckIncomingAudio();
        if (has_new_audio && (device_state_ == kDeviceStateIdle || device_state_ == kDeviceStateFatalError)) {
            ESP_LOGI(TAG, "Fetching new podcast audio...");
            ToggleChatState();
            vTaskDelay(pdMS_TO_TICKS(1000 * 60));
        } else {
...
void Application::OnAlarmTriggered(const AlarmEntry& alarm) {
    ESP_LOGI(TAG, "Alarm triggered: %02d:%02d '%s'", alarm.hour, alarm.minute, alarm.label);
    ...
    // 显示闹钟提醒
    std::string alert_message = alarm.label[0] ? alarm.label : Lang::Strings::ALARM;
    auto display = Board::GetInstance().GetDisplay();
    display->SetChatMessage("system", alert_message.c_str());

    // 开始循环播放闹钟声音，1分钟后自动停止
    StartAlarmLoop();
}

void Application::StartAlarmLoop() {
    // 停止之前的定时器（如果有的话）
    if (alarm_timer_handle_ != nullptr) {
        esp_timer_stop(alarm_timer_handle_);
    }
    ...
    // 立即播放第一次闹钟声音
    audio_service_.PlaySound(Lang::Sounds::OGG_RINGTONE);
    // 启动周期定时器，每3秒播放一次闹钟声音
    esp_timer_start_periodic(alarm_timer_handle_, 3000000); // 3秒

    // 启动一次性定时器，1分钟后自动停止闹钟
    esp_timer_start_once(alarm_stop_timer_handle_, 60000000); // 60秒
}

void Application::StopAlarmLoop() {
    if (alarm_timer_handle_ != nullptr) {
        esp_timer_stop(alarm_timer_handle_);
    }
    if (alarm_stop_timer_handle_ != nullptr) {
